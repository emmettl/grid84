...
- [x] Generic strike builder (`src/studies/strike.ts`): allocation by rule with per-target weapon caps, a timed track per weapon, attrition by hash, one compact effect per target; the alert force keeps its own 1956-specific version for now and should move onto it.
...
Candidate studies after SIOP//62, each needing its own brief first: the Cuban missile crisis gone hot ([brief written](docs/CUBA-62.md): the R-12 regiments against Florida and the south-east, the FKR cruise missiles and Lunas against Guantánamo and a landing, then the general exchange under SIOP-63), DEFCON 3 in October 1973 as a force-posture study of the seventies, with the 1969 readiness test as its second act ([brief](docs/DEFCON3-73.md); **built** at `#/study/defcon3-73`: the ladder of the night from the Foreign Relations volume's own documents, both orders of battle by script from unit histories, and `#/study/defcon3-73/execute`, SIOP-4 enacted with that force by the NUWEP-74 categories as a rule against a Soviet launch on warning; the 1969 act still to build), Able Archer 83 gone hot as the European theatre war, INF and release procedures included ([brief written](docs/ABLE-ARCHER-83.md)), Protect and Survive: Britain, from the Home Office exercises (Square Leg's 131 weapons in 1980), Openshaw, Steadman and Greene's *Doomsday* plumes, Campbell's *War Plan UK*, the JIC's declassified target assessment and the Warsaw Pact's Seven Days to the River Rhine (brief to follow), India–Pakistan after Toon and Robock (2019), the Korean peninsula. See [Beyond SIOP//62](docs/SIOP-62.md#beyond-siop62).
