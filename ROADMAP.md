...
- [x] Suborbital trajectories: ballistic sorties carry a height per waypoint from the minimum-energy model's apogee, the vertex buffers carry it as an attribute, and the shaders project through MapLibre's `projectTileFor3D`, metres above the sphere on the globe and metres through the custom-layer matrix on the plane, with the horizon clip applied to the elevated position. Any study whose tracks leave the surface draws through the WebGL layer. The labels follow the ground track, not the arc.
