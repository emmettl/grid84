...
- [x] Generic strike builder (`src/studies/strike.ts`): allocation by rule with per-target weapon caps, a timed track per weapon, attrition by hash or calibrated to a documented assurance, cruise routes through refuelling areas by hook, a burst and plume assumption per target, one compact effect per target. Every study's strike runs through it, the SIOP//62 alert force included; the Soviet response of 1961 is the one strike still written out by hand.
