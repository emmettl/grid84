...
export function buildStrikeStudy(plan: StrikePlan, wind: WindAloft, countdownSeconds = 5): Study {
  ...
  const entities: Entity[] = [
    ...
    { kind: 'site', id: 'target-site', name: target.name, designation: `${designation.role.toUpperCase()} ${designation.code} · ${classification.category}`, label: true, position: target.position, evidence: 'documented', provenance: { source: 'OpenStreetMap via Photon' }, facts: [{ label: 'Population within 10 km', value: classification.population > 0 ? `${Math.round(plan.classification.population).toLocaleString('en-GB')} within 30 km on the 2025 grid` : 'None on the grid', evidence: 'modelled', provenance: { source: 'GHSL GHS-POP R2023A, 2025 epoch' } }] },
    ...strike.entities,
  ]
