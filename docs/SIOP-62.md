...
| **Reconstructed** | Derived from documented facts by a stated method | Solid vectors in lilac | Bomber routes as great circles with refuelling tracks; which 1956 targets survived into the 1961 target list |
...
| **Readiness clock** | **Built:** `#/lab/readiness`. The fourteen options as a force-generation curve from 1,004 systems at H-hour to 2,244 at 14 hours, the 15 July 1961 force tables by command and system, reaction times, the launch sequence, the 1961 posture, and the DEFCON ladder; a preparation-time slider reads the option, force and megatonnage | The briefing's four stated points and the table totals, both in the tests; the chart's 1,685 alert weapons against the table's 1,530 carried side by side | Straight lines between documented points; option numbers between the stated ones inferred from the n at n−1 hours pattern |
...
| **Terrain shock** | **Built:** `#/lab/terrain`. Blast propagation over real terrain rather than a plane: Nagasaki's Urakami valley confined and channelled the blast, and Glasstone treats terrain only qualitatively, which is why planar tools stop there. Three fidelities in order: line-of-sight shadowing of the burst point over the terrain tiles; a 2D acoustic wave solver over the height field on a small grid, run in a shader when the camera is close, showing reflections and channelling; and the planar model everywhere else. Real CFD is out of scope | Nagasaki damage surveys against the shadowed and channelled zones; the planar radii in the far field | A linear wave over a 2.5D surface; qualitative reflections, not pressures anyone should quote |
| **Fallout plume** | **Built:** `#/lab/fallout`. Glasstone & Dolan Table 9.93 unit-time contours scaled by yield, fission fraction and wind; t^-1.2 decay integrated from arrival; Table 12.108 acute mortality over the HYDE grid | Glasstone's §9.99 worked example (10 Mt, 30 mph) and Table 9.19 decay, both in the tests; Castle Bravo preset for Rongelap | Single wind vector, no shear, smooth plane, no shelter |
...
**Built, 9 September 2026.** The proof runs. The Atlas D launches at H+15:00 on the documented reaction time, flies a minimum-energy arc of 21.6 minutes to apogee 1,026 km, and detonates at H+36:38 with the prompt-effects rings drawn on the Anadyr estuary; the camera descends to the target in the terminal phase. The Chrome Dome B-52 flies its reconstructed loop throughout, passing Thule half an hour before H-hour, its SIOP target logged as withheld. Every mark opens a provenance panel with its facts badged by tier. Population exposure is shown as not computed. Population exposure now computes at impact from the HYDE 1961 grid: the Anadyr weapon gives about 7,000 dead by the blast-only 1961 method and 8,500 with mass fire, both labelled modelled with their sources. A burst switch makes the weapon a contact surface burst: the overpressure rings shrink to the contact-burst radii (Glasstone §3.34 with the Kinney & Graham free-air fit), the blast-only count falls to about 6,600, a plume grows downwind on the clock under an assumed 15 mph wind, and four days after impact the log reports about 39,000 people under the 1 rad/hr contour and 2,900 acute fallout deaths with no shelter, badged inferred. Not yet built: the Soviet response.
...
| --- | --- | --- |
| **Contemporary single weapon** | Glasstone and Dolan; GHSL population; NUKEMAP as the validation reference | The base case, and the main thing people use NUKEMAP for. Built as the population lab with the GHSL grid: 30-arc-second tiles fetched on demand, finer than the inner rings, so the cell-resolution caveat that governs every HYDE readout falls away. NUKEMAP comparison run and recorded in [VALIDATION.md](VALIDATION.md#nukemap-comparison): same method, a third's difference in the 5 psi radius from the burst-height convention, agreement within a fifth at fifty megatons. |
...
| **The Cuban missile crisis gone hot** · [brief](CUBA-62.md) · built | Norris and Kristensen's 2012 nuclear order of battle; the National Security Archive's Anadyr and crisis collections; Dobbs, *One Minute to Midnight*; Nash on the Jupiters | Three acts on one map: the R-12 regiments against Florida and the south-east, the FKRs and Lunas against Guantánamo and a landing, then SIOP-63 at DEFCON 2. The forward-based Jupiters in Turkey and Italy and the Thors in Britain are the Soviet side's first targets and the trade that ended the crisis. |
| **DEFCON 3, October 1973** · [brief](DEFCON3-73.md) · built | National Security Archive EBB 98; Sagan, "Nuclear Alerts and Crisis Management" (1985); Blechman and Hart (1982); the Nuclear Weapons Databook volumes 1 and 4; Burr on NSDM-242 | A posture held rather than a plan executed: the only worldwide alert after Cuba, climbed step by step from the record of one night, with RUN leading into SIOP-4 three months before the doctrine that replaced it. The 1969 secret alert is its second act. |
| **Able Archer 83 gone hot** · [brief](ABLE-ARCHER-83.md) · built | The National Security Archive's [Able Archer 83 Sourcebook](https://nsarchive.gwu.edu/project/able-archer-83-sourcebook), over a thousand pages assembled by Nate Jones; the 1990 PFIAB report *The Soviet "War Scare"* released after a twelve-year fight; Jones, *Able Archer 83* (2016) | A counterfactual on a documented exercise, with both sides' readiness states in the record. The withheld tier would be busy: the 2025 briefing book is titled *The Censored History of Able Archer 83*. |
| **Protect and Survive: Britain** · [brief](BRITAIN-80.md) · built | The Home Office's Square Leg (1980) and Hard Rock (1982) exercises; Openshaw, Steadman and Greene, *Doomsday: Britain after Nuclear Attack* (1983); Campbell, *War Plan UK* (1982); the JIC's declassified assessment of probable Soviet targets; the Warsaw Pact's Seven Days to the River Rhine (1979, released 2005) | British scholarship drew the fallout plumes over real counties forty years ago; the fallout lab was built for that geometry. Both sides' plans are partly in the record, so the withheld tier and the documented tier sit next to each other on one small island. |
...
| Blast-only dead over HYDE 1961, as a union | 53 million; 61 million summed per target; the delivered force strikes 1,164 of the 1,312 targets | JCS 1961 estimate for the alert force: 80 million Soviet dead, 37 percent |
| With mass fire, Postol bound, as a union | 110 million; 140 million summed per target | |
...
**The sum.** Each side's headline is a union over the population grid rather than a sum over targets: every grid sample is claimed once, by the most severe blast band any detonation reaches it with, and flagged if any fire zone covers it; once a plume has fallen, the sample carries the highest dose any plume gives it. Deaths are taken in sequence per sample, blast by the 1961 bands, then fire as Postol's bound, then acute fallout among those the first two left alive, and the readout's last cell is all three together, one minus the product of the survivals. The per-target lines in the log and the provenance panel are still summed separately, and the readout shows what the sum would have said, so the size of the overlap is visible: for the alert force, 61 million by blast summed against 53 million in the union.

**Rendering.** A study with more than a hundred tracks draws them through a WebGL custom layer rather than GeoJSON sources ([`src/map/track-layer.ts`](../src/map/track-layer.ts)). Every track is densified once into a static vertex buffer with the time it reaches each vertex; a study tick rewrites only an index list of the flown segments, one head segment per moving vehicle, and one point per vehicle. Projection is MapLibre's own shader prelude, so the lines follow the globe and its transition to mercator. The evidence grammar survives the change: each tier keeps its colour, vehicle marks keep their dark fill, tier-coloured ring and halo, and aircraft draw as a silhouette turned to the track's heading so a bomber stream reads apart from a missile salvo. Ballistic sorties are drawn as trajectories rather than ground tracks: each carries the height of the minimum-energy arc at every point, an Atlas to Anadyr rising past a thousand kilometres, and the layer projects the height through MapLibre's three-dimensional projection under both the globe and the plane, so an arc seen from a pitched camera leaves the surface and comes back to it, and the vehicle's label rides up with it. On the alert force this costs 1 to 6 ms of main-thread time per update against 65 to 95 ms just to build the GeoJSON features it replaces.
