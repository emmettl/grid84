...
The assumption underneath is that nobody moves: the dose is taken by the population where the grid has it, with no sheltering and no evacuation. In a small attack that would be a serious overstatement. In the attacks these studies draw it is close to the case, since the roads out are in the blast areas, the plume outruns a car along most axes, and the people under the heaviest contours take a lethal dose within the first hours. The readout states it as an omission on every surface-burst study rather than leaving it to be inferred.
