...
- **The matrix.** `table.ts` computes, once per browser, the dead from one detonation on every target the search can strike, at each yield class the force carries (100, 335 and 1,100 kt for the American weapons; 500 and 1,000 kt for the Soviet), by the studies' method: the DCPA bands for blast and Postol's bound for fire, through the exposure workers over the 1985 grid. Three silos per wing stand for the field. A second pass lays each target's assigned number of weapons down over its area, a sunflower whose spacing lets the 5 psi rings meet, and counts every person once through the union, so the row carries both figures (Leningrad: 1.5 M from one W78, 4.0 M from ten). About two thousand exposure jobs and seven hundred unions, a minute or so; kept in `localStorage` under `grid84-wopr-table-v2`; *Recalibrate* clears it.
